...
const LTVValueCard: React.FC<LTVValueCardProps> = (
  ...
  return (
    <>
      <div
        className={`p-6 rounded-2xl border border-nomura-light-grey box-shadow-card flex flex-1 max-w-full ${className}`}
      >
            ...
            {data.map((item, i) => {
              ...
              return (
                <div
                  ...
                  } justify-between border-dashed border-b-2 border-noumura-grey p-1`}
                >
                  <div
                    className={`nomura-16px-regular text-nomura-dark-grey w-full ${
                      i === 0 && cardValue == "1" ? "font-bold" : ""
                    }`}
                  >
                    {label}
                  </div>
