...
import { toCommaSeprated, toSetCommaFormatPercentage } from "@/_utils/stringUtils";

export const isOverride = (ltvCalculation: any) => {
    return ltvCalculation?.overrideCalculationResult && ltvCalculation?.overrideCalculationResult?.hasOverride;
};


export const toEquityHeaderData = (result?: EquityLTVCalculationRes): HeaderData => {
    ...
    return {
        ...
        result: "LTV Result",
        ticker: `${result?.ticker} ${result?.takaraData?.equityExchangeMarket}` ?? "-",
    };
};
...
export const toLTVValuesData = (result?: EquityLTVCalculationRes): DisplayItem[] => {
    const ltvCalculation = result?.ltvCalculation;

    const isOverride =
        ltvCalculation?.overrideCalculationResult && ltvCalculation?.overrideCalculationResult?.hasOverride;
...
export const toOverrideData = (result?: EquityLTVCalculationRes): OverrideData => {
    const ltvCalculation = result?.ltvCalculation;
